...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        call_frame = procedure.env.make_call_frame(procedure.formals, args)
...
def do_or_form(vals, env):
    ...
    if len(vals) == 0:
        return False
    for i in range(len(vals) - 1):
        curr_val = scheme_eval(vals[i], env)
        if scheme_true(curr_val):
            return quote(curr_val)
    return vals[len(vals) - 1]
    ...
    while True:
        ...
        if (scheme_symbolp(first) # first might be unhashable
            ...
            if isinstance(procedure, PrimitiveProcedure):
                ...
                call_frame = procedure.env.make_call_frame(procedure.formals, args)
                expr, env = procedure.body, call_frame
            elif isinstance(procedure, MuProcedure):
                call_frame = env.make_call_frame(procedure.formals, args)
                expr, env = procedure.body, call_frame
            else:
