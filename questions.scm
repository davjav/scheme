...
(define (zip pairs)
  (cond
    ((null? pairs) (list () ()))
    (else (list (cons (caar pairs) (car (zip (cdr pairs))))
...
(define (list-partitions total max-pieces max-value)
  (cond
    ...
    ((= max-value 0) nil)
    ((= max-pieces 0) nil)
    (else (begin (define yes (list-partitions (- total max-value) (- max-pieces 1) max-value))
      (define no (list-partitions total max-pieces (- max-value 1)))
      (define add_m (lambda (s) (cons max-value s))) 
      (define yes (apply-to-all add_m yes)) 
      (accumulate cons no yes))))
  )
